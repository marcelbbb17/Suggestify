...
def proxy_api_request(current_user):
    try:
        ...
        url = f"{base_url}?api_key={TMBD_KEY}&language=en-US"
...
def get_watchlist(current_user):
    ...
    try:
        ...
        for item in watchlist_items:
            # Looks at the stored recommendations first (prevents unnecessary api calls)
            cursor.execute("""
            ...
            if not movie_details:
                ...
                try:
                    if movie_details['genres']:
                        item['genres'] = ast.literal_eval(movie_details['genres'])
                    else:
                        item['genres'] = []
                except:
                    item['genres'] = []
...
def add_to_watchlist(current_user):
    ...
    try:
        ...
        connection.commit()
        return jsonify({"success": "Movie have been successfully added to watchlist"}), 201
    except Exception as e:
        connection.rollback()
        print(f"An error occured when adding to watchlist: {str(e)}")
        return jsonify({"error": "Failed to add movie to watchlist"}), 500
    finally:
        cursor.close()
        connection.close()        

@app.route("/watchlist/<int:movie_id>", methods=["PUT"])
@token_required
def update_watchlist(current_user, movie_id):
    ...
    try:
        ...
        connection.rollback()
        print(f"failed to uodate watchlist: {str(e)}")
        return jsonify({"error": "An error occured when updating the watchlist"}), 500
    finally:
        cursor.close()
        connection.close()
...
@token_required
def delete_watchlist(current_user, movie_id):
    connection = get_db_connection()
    cursor = connection.cursor()
    ...
    try:
        ...
        connection.commit()
        return jsonify({"success": "Movies were removed from watchlist"}), 200
    except Exception as e:
        connection.rollback()
        print(f"An error occured when removing movie from watchlist: {str(e)}")
...
@token_required
def search_for_movies(current_user):
    query = request.args.get('query', "")

    if not query:
        return jsonify({"error" : "Search query is too short"}), 400
    
    search = f"https://api.themoviedb.org/3/search/movie?api_key={TMBD_KEY}&language=en-US&query={query}&page=1&include_adult=false"
        ...
        if response.status_code == 200:
            ...
            for movie in results:
                ...
                for genre_id in genre_ids:
                    genre_name = genre_mapping.get(genre_id, "unkown")
                    movie_genre.append(genre_name)
                
                movie["genres"] = movie_genre
                movie.pop("genre_ids", None)

                # Gets actors of each movie 
                movie["actors"] = get_actors(movie["id"])
            print([movie["title"] for movie in results ])
            return jsonify({"results": results}), 200
        else:
            return jsonify({"error": "Failed to get movies"}), 500
    except Exception as e:
        print(f"An error occured when fetching movies: {str(e)} ")
        return jsonify({"error" : "An error occurec when fetching movies"}), 500
